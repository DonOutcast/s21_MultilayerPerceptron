...
auto s21::Facade::feed_forward() -> void {
...
auto s21::Facade::get_result() -> size_t {
...
auto s21::Facade::train_network(const std::string file_name, const size_t epochs) -> std::vector<double> {
    return this->m_net_->train_network(file_name, epochs);
}

auto s21::Facade::cross_validation(const std::string &file_name, const size_t k) -> std::vector<double> {
